...
class Helper_Songs_Add extends CoreHelper {    
    public function run() {
        ...
        Core::get('DB')->query("INSERT INTO voting_list (trackid, addedBy, addedDate) VALUES (?, ?, NOW())", array($trackID, $this->parent->LoggedUser['userid']));
        
        //Add a vote
        Core::get('DB')->query("INSERT INTO votes (trackid, userid, updown) VALUES (?, ?, 1)", array($trackID, $this->parent->LoggedUser['userid']));
        ...
        $Track["position"] = $RowInfo["row_position"];

        $msgData = array('event'=>'add', 'data' => $Track);
        $msg = WebSocketMessage::create(json_encode($msgData));
        
        Core::requireLibrary("wesocket.client.php", "phpws");
