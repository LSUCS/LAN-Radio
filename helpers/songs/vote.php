...
class Helper_Songs_Vote extends CoreHelper {    
    public function run() {
        ...
        Core::get('DB')->query("SELECT updown FROM votes WHERE trackid = ? AND userid = ?", array($trackID, $User->ID));
        if(Core::get('DB')->record_count()) {
            list($vote) = Core::get('DB')->next_record(MYSQLI_NUM);
            if($vote == $direction) die('identical');
            
            Core::get('DB')->query("UPDATE votes SET updown = ? WHERE trackid = ? AND userid = ?", array($direction, $trackID, $User->ID));
        } else {
            Core::get('DB')->query("INSERT INTO votes (trackid, userid, updown) VALUES (?, ?, ?)", array($trackID, $User->ID, $direction));
        }
